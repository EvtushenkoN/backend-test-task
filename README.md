...
### Выполнено
- Сделаны сущности и репозитории для стран, купонов и товаров
- Сделан сервис для работы с платежными процессорами, который может расширяться логикой для новых процессоров. Возможно потребуются отдельные сервисы для каждого
- Сделан пример юнит-теста для этого сервиса
- Валидация налоговых номеров производится через регексы, которые хранятся в базе - решением лучше было бы в базе держать понятный человеку/менеджеру формат вида XXYY11111 и по нему строить регекс
- Создан дополнительный контейнер для базы данных
- Для запуска фикстур и миграций можно сделать make populate после инициализации проекта
